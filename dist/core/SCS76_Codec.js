...
export class SCS76Codec {
    ...
    static encode(x, y, lastX, lastY, lastVx, lastVy, snapshotId, ackInputSeq, timestamp, forceKeyframe = false, deltaTimeMs = 16.66) {
        ...
        if (isKeyframe)
            mode = 0;
        else if (adx < zeroThreshold && ady < zeroThreshold)
            mode = 3;
        else if (adx < 1.27 && ady < 1.27)
            mode = 2;
        else if (adx < 327.67 && ady < 327.67)
            mode = 1;
        else
            mode = 0;
        let flags = mode & 0x03;
    ...
    static decode(buffer, lastPos, lastVel, deltaTimeMs = 16.66) {
        if (buffer.length < 8)
            return null;
        const version = buffer[0];
        if (version !== this.VERSION_76 && version !== this.VERSION_77)
            return null;
        ...
        if (buffer.length < headerSize + payloadSize)
            return null;
...
_a = SCS76Codec;
SCS76Codec.VERSION_76 = 0x76;
SCS76Codec.VERSION_77 = 0x77;
SCS76Codec.CURRENT_VERSION = _a.VERSION_77;
SCS76Codec.HEADER_SIZE_V76 = 8;
SCS76Codec.HEADER_SIZE_V77 = 10;
SCS76Codec.SCALE = 100.0;
